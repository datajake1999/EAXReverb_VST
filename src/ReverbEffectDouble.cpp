...
inline double FilterStateDouble_Process(FilterStateDouble *filter, double sample)
{
    double outsmp;
...
void FilterStateDouble_clear(FilterStateDouble *filter)
{
    filter->x[0] = 0.0f;
    filter->x[1] = 0.0f;
    filter->y[0] = 0.0f;
...
void FilterStateDouble_setParams(FilterStateDouble *filter, FilterTypeDouble type, double gain, double freq_mult, double bandwidth)
{
    double alpha;
    double w0;
...
void ReverbEffectDouble::ComputeAmbientGains(double ingain, double gains[OUTPUT_CHANNELS])
{
    uint32_t i;
...
void ReverbEffectDouble::ComputeDirectionalGains(const double dir[3], double ingain, double gains[OUTPUT_CHANNELS])
{
...
static inline double DelayLineDoubleOut(DelayLineDouble *Delay, uint32_t offset)
{
    return Delay->Line[offset&Delay->Mask];
}

static inline void DelayLineDoubleIn(DelayLineDouble *Delay, uint32_t offset, double in)
{
    Delay->Line[offset&Delay->Mask] = in;
}

// Attenuated delay line output routine.
static inline double AttenuatedDelayLineDoubleOut(DelayLineDouble *Delay, uint32_t offset, double coeff)
{
    return coeff * Delay->Line[offset&Delay->Mask];
}

// Basic attenuated all-pass input/output routine.
static inline double AllpassInOut(DelayLineDouble *Delay, uint32_t outOffset, uint32_t inOffset, double in, double feedCoeff, double coeff)
{
    double out, feed;

    out = DelayLineDoubleOut(Delay, outOffset);
    feed = feedCoeff * in;
    DelayLineDoubleIn(Delay, inOffset, (feedCoeff * (out - feed)) + in);
...
inline double ReverbEffectDouble::EAXModulation(double in)
{
    double sinus, frac;
    uint32_t offset;
    double out0, out1;
    ...
    out0 = DelayLineDoubleOut(&this->Mod.Delay, this->Offset - offset);
    out1 = DelayLineDoubleOut(&this->Mod.Delay, this->Offset - offset - 1);
    DelayLineDoubleIn(&this->Mod.Delay, this->Offset, in);
...
inline double ReverbEffectDouble::EarlyDelayDoubleLineDoubleOut(uint32_t index)
{
    return AttenuatedDelayLineDoubleOut(&this->Early.Delay[index], this->Offset - this->Early.Offset[index], this->Early.Coeff[index]);
}

// Given an input sample, this function produces four-channel output for the  early reflections.
inline void ReverbEffectDouble::EarlyReflection(double in, double *out)
{
    double d[4], v, f[4];

    // Obtain the decayed results of each early delay line.
    d[0] = this->EarlyDelayDoubleLineDoubleOut(0);
    d[1] = this->EarlyDelayDoubleLineDoubleOut(1);
    d[2] = this->EarlyDelayDoubleLineDoubleOut(2);
    d[3] = this->EarlyDelayDoubleLineDoubleOut(3);
    ...
    DelayLineDoubleIn(&this->Early.Delay[0], this->Offset, f[0]);
    DelayLineDoubleIn(&this->Early.Delay[1], this->Offset, f[1]);
    DelayLineDoubleIn(&this->Early.Delay[2], this->Offset, f[2]);
    DelayLineDoubleIn(&this->Early.Delay[3], this->Offset, f[3]);
...
inline double ReverbEffectDouble::LateAllPassInOut(uint32_t index, double in)
{
...
inline double ReverbEffectDouble::LateDelayDoubleLineDoubleOut(uint32_t index)
{
    return AttenuatedDelayLineDoubleOut(&this->Late.Delay[index], this->Offset - this->Late.Offset[index], this->Late.Coeff[index]);
}

// Low-pass filter input/output routine for late reverb.
inline double ReverbEffectDouble::LateLowPassInOut(uint32_t index, double in)
{
...
inline void ReverbEffectDouble::LateReverb(const double *in, double *out)
{
    double d[4], f[4];
    ...
    // to 0.
    d[0] = this->LateLowPassInOut(2, in[2] + this->LateDelayDoubleLineDoubleOut(2));
    d[1] = this->LateLowPassInOut(0, in[0] + this->LateDelayDoubleLineDoubleOut(0));
    d[2] = this->LateLowPassInOut(3, in[3] + this->LateDelayDoubleLineDoubleOut(3));
    d[3] = this->LateLowPassInOut(1, in[1] + this->LateDelayDoubleLineDoubleOut(1));
    ...
    DelayLineDoubleIn(&this->Late.Delay[0], this->Offset, f[0]);
    DelayLineDoubleIn(&this->Late.Delay[1], this->Offset, f[1]);
    DelayLineDoubleIn(&this->Late.Delay[2], this->Offset, f[2]);
    DelayLineDoubleIn(&this->Late.Delay[3], this->Offset, f[3]);
}

// Given an input sample, this function mixes echo into the four-channel late
// reverb.
inline void ReverbEffectDouble::EAXEcho(double in, double *late)
{
    double out, feed;

    // Get the latest attenuated echo sample for output.
    feed = AttenuatedDelayLineDoubleOut(&this->Echo.Delay,
    ...
    DelayLineDoubleIn(&this->Echo.Delay, this->Offset, feed);
}


// Perform the EAX reverb pass on a given input sample, resulting in four-channel output.
inline void ReverbEffectDouble::EAXVerbPass(double in, double *early, double *late)
{
    double feed, taps[4];

    // Low-pass filter the incoming sample.
    in = FilterStateDouble_Process(&this->LpFilter, in);
    in = FilterStateDouble_Process(&this->HpFilter, in);

    // Perform any modulation on the input.
    in = this->EAXModulation(in);

    // Feed the initial delay line.
    DelayLineDoubleIn(&this->Delay, this->Offset, in);

    // Calculate the early reflection from the first delay tap.
    in = DelayLineDoubleOut(&this->Delay, this->Offset - this->DelayTap[0]);
    this->EarlyReflection(in, early);

    // Feed the decorrelator from the energy-attenuated output of the second
    // delay tap.
    in = DelayLineDoubleOut(&this->Delay, this->Offset - this->DelayTap[1]);
    feed = in * this->Late.DensityGain;
    DelayLineDoubleIn(&this->Decorrelator, this->Offset, feed);

    // Calculate the late reverb from the decorrelator taps.
    taps[0] = feed;
    taps[1] = DelayLineDoubleOut(&this->Decorrelator, this->Offset - this->DecoTap[0]);
    taps[2] = DelayLineDoubleOut(&this->Decorrelator, this->Offset - this->DecoTap[1]);
    taps[3] = DelayLineDoubleOut(&this->Decorrelator, this->Offset - this->DecoTap[2]);
...
void ReverbEffectDouble::Process(uint32_t SamplesToDo, const double *SamplesIn, double *SamplesOut)
{
...
static inline void RealizeLineOffset(double *sampleBuffer, DelayLineDouble *Delay)
{
    Delay->Line = &sampleBuffer[(ptrdiff_t)Delay->Line];
}

// Calculate the length of a delay line and store its mask and offset.
static uint32_t CalcLineLength(double length, ptrdiff_t offset, uint32_t frequency, DelayLineDouble *Delay)
{
    uint32_t samples;
...
void ReverbEffectDouble::AllocLines(uint32_t frequency)
{
...
void ReverbEffectDouble::UpdateModulatorDouble(double modTime, double modDepth, uint32_t frequency)
{
    uint32_t range;
...
void ReverbEffectDouble::UpdateDelayLineDouble(double earlyDelay, double lateDelay, uint32_t frequency)
{
...
void ReverbEffectDouble::UpdateEarlyLines(double reverbGain, double earlyGain, double lateDelay)
{
    uint32_t index;
...
void ReverbEffectDouble::UpdateDecorrelator(double density, uint32_t frequency)
{
    uint32_t index;
    double length;
...
void ReverbEffectDouble::UpdateLateLines(double reverbGain, double lateGain, double xMix, double density, double decayTime, double diffusion, double hfRatio, double cw, uint32_t frequency)
{
    double length;
    uint32_t index;
...
void ReverbEffectDouble::UpdateEchoLine(double reverbGain, double lateGain, double echoTime, double decayTime, double diffusion, double echoDepth, double hfRatio, double cw, uint32_t frequency)
{
...
void ReverbEffectDouble::Update3DPanning(const double *ReflectionsPan, const double *LateReverbPan, double Gain)
{
...
void ReverbEffectDouble::Update(int frequency)
{
    ...
    FilterStateDouble_setParams(&this->LpFilter, Filter_HighShelf_Double, this->settings.GainHF, hfscale, 0.0f);

    lfscale = this->settings.LFReference / frequency;
    FilterStateDouble_setParams(&this->HpFilter, Filter_LowShelf_Double, this->settings.GainLF, lfscale, 0.0f);

    // Update the modulator line.
    this->UpdateModulatorDouble(this->settings.ModulationTime, this->settings.ModulationDepth, frequency);

    // Update the initial effect delay.
    this->UpdateDelayLineDouble(this->settings.ReflectionsDelay, this->settings.LateReverbDelay, frequency);
...
void ReverbEffectDouble::Create(uint32_t frequency)
{
    ...
    FilterStateDouble_clear(&this->LpFilter);
    FilterStateDouble_clear(&this->HpFilter);
...
void ReverbEffectDouble::Destroy()
{
